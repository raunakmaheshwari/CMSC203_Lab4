   ...
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	  for (int row = 0; row < intArray.length; row++) {
		  ...
		  if (intArray[row][column] < minNum) {
			  minRow = row;
			  minNum = intArray[row][column];
	  ...
	  animate(minRow, column, minNum);
	  
      return minNum; // replace this line with your return statement
   }
